...
const getTodaysRecords = async (clinician) => {
    try {
        ...
        var formattedStart = moment(start).tz('Australia/Melbourne').format("DD/MM/YY")
        ...
        for (const patient of patients) {
            ...
            for (const threshold of patient.thresholds) {
                if ((threshold.lower != -1) && (threshold.upper != -1)){
            ...
            for (const record of patient.records) {
                if (moment(record.created_at).tz('Australia/Melbourne').format("DD/MM/YY") == formattedStart) {
